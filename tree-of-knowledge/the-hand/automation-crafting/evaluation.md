# Automation Crafting — Evaluation

## Branch
**The Hand — Creation & Power**

## Family
Automation Crafting

## Variation
Evaluation

## Tier
Bronze

## Purpose
Craft automation that eliminates toil.

## When to Use
- When automation crafting is required at evaluation level
- When complexity demands evaluation approach to automation crafting
- When standard methods fail and evaluation is needed

## How to Apply
1. **Assess**: Evaluate the situation for automation crafting requirements
2. **Prepare**: Gather resources and knowledge for evaluation automation crafting
3. **Execute**: Apply evaluation techniques for automation crafting
4. **Verify**: Confirm outcomes match automation crafting objectives
5. **Iterate**: Refine based on results. Document evaluation learnings.

## Key Principles
- Always verify before trusting in automation crafting
- Start with fundamentals before evaluation techniques
- Document everything for future automation crafting reference
- Seek feedback on evaluation automation crafting approach
- Continuously improve automation crafting capability

## Common Pitfalls
- Assuming evaluation level without proper foundation
- Ignoring context when applying automation crafting
- Failing to verify automation crafting outcomes

## Related Skills
- The Light — related branch
- Reality Sculpting — sibling family
- Automation Crafting — Expert-Level — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-hand-automation-crafting-evaluation
Branch: the-hand
Family: automation-crafting
Variation: Evaluation
Tier: Bronze
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
