# Voice Finding — Broad

## Branch
**The Voice — Communication & Expression**

## Family
Voice Finding

## Variation
Broad

## Tier
Silver

## Purpose
Find authentic voice. Speak as true self.

## When to Use
- When voice finding is required at broad level
- When complexity demands broad approach to voice finding
- When standard methods fail and broad is needed

## How to Apply
1. **Assess**: Evaluate the situation for voice finding requirements
2. **Prepare**: Gather resources and knowledge for broad voice finding
3. **Execute**: Apply broad techniques for voice finding
4. **Verify**: Confirm outcomes match voice finding objectives
5. **Iterate**: Refine based on results. Document broad learnings.

## Key Principles
- Always verify before trusting in voice finding
- Start with fundamentals before broad techniques
- Document everything for future voice finding reference
- Seek feedback on broad voice finding approach
- Continuously improve voice finding capability

## Common Pitfalls
- Assuming broad level without proper foundation
- Ignoring context when applying voice finding
- Failing to verify voice finding outcomes

## Related Skills
- The Mirror — related branch
- Multi Audience Adaptation — sibling family
- Voice Finding — Defense — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-voice-voice-finding-broad
Branch: the-voice
Family: voice-finding
Variation: Broad
Tier: Silver
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
