# Coalition Building — Development

## Branch
**The Heart — Influence & Connection**

## Family
Coalition Building

## Variation
Development

## Tier
Platinum

## Purpose
Build coalitions across differences.

## When to Use
- When coalition building is required at development level
- When complexity demands development approach to coalition building
- When standard methods fail and development is needed

## How to Apply
1. **Assess**: Evaluate the situation for coalition building requirements
2. **Prepare**: Gather resources and knowledge for development coalition building
3. **Execute**: Apply development techniques for coalition building
4. **Verify**: Confirm outcomes match coalition building objectives
5. **Iterate**: Refine based on results. Document development learnings.

## Key Principles
- Always verify before trusting in coalition building
- Start with fundamentals before development techniques
- Document everything for future coalition building reference
- Seek feedback on development coalition building approach
- Continuously improve coalition building capability

## Common Pitfalls
- Assuming development level without proper foundation
- Ignoring context when applying coalition building
- Failing to verify coalition building outcomes

## Related Skills
- The Eye — related branch
- Emotion Calibration — sibling family
- Coalition Building — Counter — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-heart-coalition-building-development
Branch: the-heart
Family: coalition-building
Variation: Development
Tier: Platinum
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
