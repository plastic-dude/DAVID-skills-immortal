# Coalition Building — Logistics

## Branch
**The Heart — Influence & Connection**

## Family
Coalition Building

## Variation
Logistics

## Tier
Bronze

## Purpose
Build coalitions across differences.

## When to Use
- When coalition building is required at logistics level
- When complexity demands logistics approach to coalition building
- When standard methods fail and logistics is needed

## How to Apply
1. **Assess**: Evaluate the situation for coalition building requirements
2. **Prepare**: Gather resources and knowledge for logistics coalition building
3. **Execute**: Apply logistics techniques for coalition building
4. **Verify**: Confirm outcomes match coalition building objectives
5. **Iterate**: Refine based on results. Document logistics learnings.

## Key Principles
- Always verify before trusting in coalition building
- Start with fundamentals before logistics techniques
- Document everything for future coalition building reference
- Seek feedback on logistics coalition building approach
- Continuously improve coalition building capability

## Common Pitfalls
- Assuming logistics level without proper foundation
- Ignoring context when applying coalition building
- Failing to verify coalition building outcomes

## Related Skills
- The Light — related branch
- Emotion Calibration — sibling family
- Coalition Building — Creation — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-heart-coalition-building-logistics
Branch: the-heart
Family: coalition-building
Variation: Logistics
Tier: Bronze
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
