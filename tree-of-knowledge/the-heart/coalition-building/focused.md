# Coalition Building — Focused

## Branch
**The Heart — Influence & Connection**

## Family
Coalition Building

## Variation
Focused

## Tier
Gold

## Purpose
Build coalitions across differences.

## When to Use
- When coalition building is required at focused level
- When complexity demands focused approach to coalition building
- When standard methods fail and focused is needed

## How to Apply
1. **Assess**: Evaluate the situation for coalition building requirements
2. **Prepare**: Gather resources and knowledge for focused coalition building
3. **Execute**: Apply focused techniques for coalition building
4. **Verify**: Confirm outcomes match coalition building objectives
5. **Iterate**: Refine based on results. Document focused learnings.

## Key Principles
- Always verify before trusting in coalition building
- Start with fundamentals before focused techniques
- Document everything for future coalition building reference
- Seek feedback on focused coalition building approach
- Continuously improve coalition building capability

## Common Pitfalls
- Assuming focused level without proper foundation
- Ignoring context when applying coalition building
- Failing to verify coalition building outcomes

## Related Skills
- The Mind — related branch
- Loyalty Cultivation — sibling family
- Coalition Building — Broad — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-heart-coalition-building-focused
Branch: the-heart
Family: coalition-building
Variation: Focused
Tier: Gold
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
