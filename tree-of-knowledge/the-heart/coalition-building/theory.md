# Coalition Building — Theory

## Branch
**The Heart — Influence & Connection**

## Family
Coalition Building

## Variation
Theory

## Tier
Gold

## Purpose
Build coalitions across differences.

## When to Use
- When coalition building is required at theory level
- When complexity demands theory approach to coalition building
- When standard methods fail and theory is needed

## How to Apply
1. **Assess**: Evaluate the situation for coalition building requirements
2. **Prepare**: Gather resources and knowledge for theory coalition building
3. **Execute**: Apply theory techniques for coalition building
4. **Verify**: Confirm outcomes match coalition building objectives
5. **Iterate**: Refine based on results. Document theory learnings.

## Key Principles
- Always verify before trusting in coalition building
- Start with fundamentals before theory techniques
- Document everything for future coalition building reference
- Seek feedback on theory coalition building approach
- Continuously improve coalition building capability

## Common Pitfalls
- Assuming theory level without proper foundation
- Ignoring context when applying coalition building
- Failing to verify coalition building outcomes

## Related Skills
- The Voice — related branch
- Movement Sparking — sibling family
- Coalition Building — Synthesis — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-heart-coalition-building-theory
Branch: the-heart
Family: coalition-building
Variation: Theory
Tier: Gold
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
