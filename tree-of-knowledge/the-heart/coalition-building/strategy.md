# Coalition Building — Strategy

## Branch
**The Heart — Influence & Connection**

## Family
Coalition Building

## Variation
Strategy

## Tier
Bronze

## Purpose
Build coalitions across differences.

## When to Use
- When coalition building is required at strategy level
- When complexity demands strategy approach to coalition building
- When standard methods fail and strategy is needed

## How to Apply
1. **Assess**: Evaluate the situation for coalition building requirements
2. **Prepare**: Gather resources and knowledge for strategy coalition building
3. **Execute**: Apply strategy techniques for coalition building
4. **Verify**: Confirm outcomes match coalition building objectives
5. **Iterate**: Refine based on results. Document strategy learnings.

## Key Principles
- Always verify before trusting in coalition building
- Start with fundamentals before strategy techniques
- Document everything for future coalition building reference
- Seek feedback on strategy coalition building approach
- Continuously improve coalition building capability

## Common Pitfalls
- Assuming strategy level without proper foundation
- Ignoring context when applying coalition building
- Failing to verify coalition building outcomes

## Related Skills
- The Voice — related branch
- Negotiation Mastery — sibling family
- Coalition Building — Logistics — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-heart-coalition-building-strategy
Branch: the-heart
Family: coalition-building
Variation: Strategy
Tier: Bronze
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
