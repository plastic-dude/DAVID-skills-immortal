# Narrative Weaving — Development

## Branch
**The Heart — Influence & Connection**

## Family
Narrative Weaving

## Variation
Development

## Tier
Bronze

## Purpose
Weave narratives that capture imagination.

## When to Use
- When narrative weaving is required at development level
- When complexity demands development approach to narrative weaving
- When standard methods fail and development is needed

## How to Apply
1. **Assess**: Evaluate the situation for narrative weaving requirements
2. **Prepare**: Gather resources and knowledge for development narrative weaving
3. **Execute**: Apply development techniques for narrative weaving
4. **Verify**: Confirm outcomes match narrative weaving objectives
5. **Iterate**: Refine based on results. Document development learnings.

## Key Principles
- Always verify before trusting in narrative weaving
- Start with fundamentals before development techniques
- Document everything for future narrative weaving reference
- Seek feedback on development narrative weaving approach
- Continuously improve narrative weaving capability

## Common Pitfalls
- Assuming development level without proper foundation
- Ignoring context when applying narrative weaving
- Failing to verify narrative weaving outcomes

## Related Skills
- The Heart — related branch
- Narrative Weaving — sibling family
- Narrative Weaving — Centralized — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-heart-narrative-weaving-development
Branch: the-heart
Family: narrative-weaving
Variation: Development
Tier: Bronze
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
