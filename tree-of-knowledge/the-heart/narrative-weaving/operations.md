# Narrative Weaving — Operations

## Branch
**The Heart — Influence & Connection**

## Family
Narrative Weaving

## Variation
Operations

## Tier
Bronze

## Purpose
Weave narratives that capture imagination.

## When to Use
- When narrative weaving is required at operations level
- When complexity demands operations approach to narrative weaving
- When standard methods fail and operations is needed

## How to Apply
1. **Assess**: Evaluate the situation for narrative weaving requirements
2. **Prepare**: Gather resources and knowledge for operations narrative weaving
3. **Execute**: Apply operations techniques for narrative weaving
4. **Verify**: Confirm outcomes match narrative weaving objectives
5. **Iterate**: Refine based on results. Document operations learnings.

## Key Principles
- Always verify before trusting in narrative weaving
- Start with fundamentals before operations techniques
- Document everything for future narrative weaving reference
- Seek feedback on operations narrative weaving approach
- Continuously improve narrative weaving capability

## Common Pitfalls
- Assuming operations level without proper foundation
- Ignoring context when applying narrative weaving
- Failing to verify narrative weaving outcomes

## Related Skills
- The Wing — related branch
- Mass Mobilization — sibling family
- Narrative Weaving — Innovation — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-heart-narrative-weaving-operations
Branch: the-heart
Family: narrative-weaving
Variation: Operations
Tier: Bronze
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
