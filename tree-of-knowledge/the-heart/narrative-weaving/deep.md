# Narrative Weaving — Deep

## Branch
**The Heart — Influence & Connection**

## Family
Narrative Weaving

## Variation
Deep

## Tier
Bronze

## Purpose
Weave narratives that capture imagination.

## When to Use
- When narrative weaving is required at deep level
- When complexity demands deep approach to narrative weaving
- When standard methods fail and deep is needed

## How to Apply
1. **Assess**: Evaluate the situation for narrative weaving requirements
2. **Prepare**: Gather resources and knowledge for deep narrative weaving
3. **Execute**: Apply deep techniques for narrative weaving
4. **Verify**: Confirm outcomes match narrative weaving objectives
5. **Iterate**: Refine based on results. Document deep learnings.

## Key Principles
- Always verify before trusting in narrative weaving
- Start with fundamentals before deep techniques
- Document everything for future narrative weaving reference
- Seek feedback on deep narrative weaving approach
- Continuously improve narrative weaving capability

## Common Pitfalls
- Assuming deep level without proper foundation
- Ignoring context when applying narrative weaving
- Failing to verify narrative weaving outcomes

## Related Skills
- The Hand — related branch
- Rapport Forging — sibling family
- Narrative Weaving — Offense — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-heart-narrative-weaving-deep
Branch: the-heart
Family: narrative-weaving
Variation: Deep
Tier: Bronze
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
