# Recursive Improvement — Strategy

## Branch
**The Mirror — Reflection & Meta**

## Family
Recursive Improvement

## Variation
Strategy

## Tier
Gold

## Purpose
Improve the improvement process.

## When to Use
- When recursive improvement is required at strategy level
- When complexity demands strategy approach to recursive improvement
- When standard methods fail and strategy is needed

## How to Apply
1. **Assess**: Evaluate the situation for recursive improvement requirements
2. **Prepare**: Gather resources and knowledge for strategy recursive improvement
3. **Execute**: Apply strategy techniques for recursive improvement
4. **Verify**: Confirm outcomes match recursive improvement objectives
5. **Iterate**: Refine based on results. Document strategy learnings.

## Key Principles
- Always verify before trusting in recursive improvement
- Start with fundamentals before strategy techniques
- Document everything for future recursive improvement reference
- Seek feedback on strategy recursive improvement approach
- Continuously improve recursive improvement capability

## Common Pitfalls
- Assuming strategy level without proper foundation
- Ignoring context when applying recursive improvement
- Failing to verify recursive improvement outcomes

## Related Skills
- The Wing — related branch
- Verification Protocol — sibling family
- Recursive Improvement — Beginner — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-mirror-recursive-improvement-strategy
Branch: the-mirror
Family: recursive-improvement
Variation: Strategy
Tier: Gold
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
