# Systems Thinking — Analysis

## Branch
**The Mind — Cognition & Knowledge**

## Family
Systems Thinking

## Variation
Analysis

## Tier
Platinum

## Purpose
Think in systems. See feedback loops.

## When to Use
- When systems thinking is required at analysis level
- When complexity demands analysis approach to systems thinking
- When standard methods fail and analysis is needed

## How to Apply
1. **Assess**: Evaluate the situation for systems thinking requirements
2. **Prepare**: Gather resources and knowledge for analysis systems thinking
3. **Execute**: Apply analysis techniques for systems thinking
4. **Verify**: Confirm outcomes match systems thinking objectives
5. **Iterate**: Refine based on results. Document analysis learnings.

## Key Principles
- Always verify before trusting in systems thinking
- Start with fundamentals before analysis techniques
- Document everything for future systems thinking reference
- Seek feedback on analysis systems thinking approach
- Continuously improve systems thinking capability

## Common Pitfalls
- Assuming analysis level without proper foundation
- Ignoring context when applying systems thinking
- Failing to verify systems thinking outcomes

## Related Skills
- The Hand — related branch
- Framework Creation — sibling family
- Systems Thinking — Fundamentals — alternate variation
- Self-evaluation protocol — meta-check
- Verification protocol — quality gate

## Verification Checklist
- [ ] Applied correctly
- [ ] Verified outcome
- [ ] Documented learnings
- [ ] Improved for next use

---
Skill ID: the-mind-systems-thinking-analysis
Branch: the-mind
Family: systems-thinking
Variation: Analysis
Tier: Platinum
Created: Thu Jul 30 07:57:47 2026
Version: 1.0
Status: ACTIVE
